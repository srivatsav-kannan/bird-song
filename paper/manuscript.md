...
Passive acoustic monitoring is one of the few survey methods that scales to rare and cryptic birds, but it depends on automated identification, and the global classifiers that support most bioacoustic monitoring do not cover most highly threatened range-restricted species. Four bird species endemic to India and listed as Endangered or Critically Endangered have sound recordings in public archives, and only one of them appears in the 6,000-species label set of BirdNET, the most widely used bird sound classifier. We assembled the complete public acoustic record of these four species from the Macaulay Library and Xeno-canto, comprising 193 unique recordings and approximately 125 minutes of audio after duplicate removal, and we evaluated whether this material can support reliable automated identification. Recordings were segmented into three-second windows screened for vocal activity, and classifiers were trained on acoustic embeddings extracted with BirdNET, including a multiple instance learning model that classifies whole recordings through an attention mechanism. Evaluation used stratified five-fold cross-validation with the unique recording as the unit of splitting, model selection confined to inner validation sets, and a stricter companion scheme that kept recordings from the same recording session on the same side of every partition. The embedding-based classifier achieved 97.4% recording-level accuracy (95% confidence interval 94.8 to 99.5, macro-F1 0.973), compared with 83.4% for a fine-tuned convolutional network, and accuracy remained at 98.9% under the session-level scheme and 92.2% under additive noise at 0 dB signal-to-noise ratio. A confidence threshold alone accepted 93.7% of 1,891 recordings of 28 non-target endemic species, and adding a background class trained on held-out species reduced this false acceptance to 13.4% while retaining 89.1% of target recordings. Public archives can therefore support accurate classifiers for these species, and open-set rejection is the main requirement remaining for field deployment.
...
India supports over 1,300 bird species, of which more than 75 occur nowhere else in the world (SoIB 2023). Endemic species with narrow geographic ranges are disproportionately vulnerable to habitat loss, and several Indian endemics are now assessed as Endangered or Critically Endangered on the IUCN Red List (IUCN 2025). Exactly four such species have sound recordings available in the major public archives. The Forest Owlet *Athene blewitti* (Endangered) was known from seven nineteenth-century specimens until its rediscovery in 1997 and retains a small, fragmented, and declining population (Rasmussen and Collar 1998). The Banasura Laughingthrush *Montecincla jerdoni* (Endangered) is confined to high-elevation shola forest patches totalling under 57 km² in the Wayanad region of the Western Ghats (Robin et al. 2017). The Bugun Liocichla *Liocichla bugunorum* (Critically Endangered), described in 2006 from Eaglenest Wildlife Sanctuary in Arunachal Pradesh, is known from a handful of territories holding at most a few tens of individuals (Athreya 2006). Jerdon's Courser *Rhinoptilus bitorquatus* (Critically Endangered), rediscovered in 1986 after 86 years without a record (Bhushan 1986), went unrecorded again from 2008 until August 2025, when its call was captured by an automated recording unit at a previously unknown site (Search for Lost Birds 2025).

These four species are natural candidates for passive acoustic monitoring. They are nocturnal or skulking, they occupy terrain where visual survey is difficult, and each has distinctive vocalisations. The reconfirmation of Jerdon's Courser in 2025 through an automated recorder illustrates the value of the approach directly. Acoustic monitoring at useful scales, however, requires automated identification, because continuous recorders produce far more audio than observers can review. The classifiers that currently dominate applied bioacoustics cannot provide this identification for these species. We verified that only the Forest Owlet is present in the label set of BirdNET V2.4 (Kahl et al. 2021), and the other three species therefore cannot be detected by it at any confidence threshold. Species-specific classifiers address a concrete operational gap.

Training classifiers for species this rare presents two methodological difficulties that shape the design of this study. The first is data scale, since the complete public acoustic record of each species ranges from 12 to 75 recordings. Recent work has shown that acoustic embeddings from large pretrained bird sound models transfer to small datasets far more effectively than training or fine-tuning conventional networks does (Ghani et al. 2023), which suggests a practical route around the scale problem. The second difficulty is evaluation validity. When all available recordings of a species come from a few recordists, sites, and seasons, a model can score highly by recognising recording conditions rather than the species, and results reported without strict separation between the data used for training, for model selection, and for testing overstate real performance. Our evaluation design addresses both the unit of independence and the separation of selection from reporting.
...
Robin, V.V., Vishnudas, C.K., Gupta, P., Rheindt, F.E., Hooper, D.M., Ramakrishnan, U. et al. (2017). Two new genera of songbirds represent endemic radiations from the Shola Sky Islands of the Western Ghats, India. *BMC Evolutionary Biology* 17, 31. https://doi.org/10.1186/s12862-017-0882-6.
