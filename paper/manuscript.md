...
The second approach makes rejection part of the classifier itself by adding a fifth output class for background species. The 28 auxiliary species were split in half, with windows from 14 species (1,077 recordings) forming the background class during training and the remaining 14 species (814 recordings) acting as the test set for the false positive rate. The test species were never seen during training, so the measured false positive rate reflects rejection of genuinely unfamiliar species.
...
*Figure 3. Recording-level confusion matrices of the embedding probe under the primary recording-level scheme (left) and the stricter event-level scheme (right).*
...
*Figure 4. UMAP projections (McInnes et al. 2018) of the window embeddings of the four target species in the raw BirdNET space (left) and after auxiliary-species contrastive adaptation (right).*
